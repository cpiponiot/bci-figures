...
fig2ab_corr_tree <- ggplot(df_ind_melt, aes(x = year, y = value, color = method)) +
  ...
  geom_segment(x = 1990, xend = 2015, y = 10, yend = 10, color = 1, 
               arrow = arrow(length = unit(0.3, "cm"), ends = "both")) +
  geom_text(x = 2000, y = 15, label = "HOM = 9 m", color = 1) +
  geom_text(data = dflabs, aes(label = label), size = 6, hjust = 4, vjust = 1,
            x= -Inf, y = Inf, inherit.aes = FALSE) +
  coord_cartesian(clip = "off") +
  expand_limits(y = 0) +
  scale_y_continuous(expand = c(0, 0)) +
  labs(y = "", x = "", title = "Individual tree level (tree #2031)") +
...
dfcorr <- subset(df_plot, !grepl("chave05|_h", method) & year >= 1985 & year < 2015 )
...
```{r, fig4-spatial, fig.height = 10, fig.width = 10, fig.cap="Crown-distributed AGB (Mg/ha; panel a) and AWP (Mg/ha/yr; panel b) on the 50 ha plot. The area outlined in red is the secondary forest section. The AGB and AWP of individual trees were distributed among 5x5m pixels in proportion to the area of intersection between the pixel and the tree crown (crown radius calculated according to Martinez Cano et al., 2019)."}
load("data/data-main-script.rda")
...
# young <- subset(bci_habitat, habitat=="young")
# create 20m quadrat correspondence for the df_crown data
...
sec_poly <- raster::rasterToPolygons(r, dissolve = TRUE) 
sec_poly <- sf::st_as_sf(sec_poly) 
sec_poly <- merge(sec_poly, unique(bci_habitat[, c("habitat", "habitat_f")]))

gagb <- ggplot(df_crown) + 
  geom_raster(aes(x = x, y = y, fill = agb)) +
  geom_sf(data = subset(sec_poly, habitat == "young"), col=2, size = 1.2, fill = NA) +
...
gawp <- ggplot(df_crown) + 
  ...
  geom_sf(data = subset(sec_poly, habitat == "young"), size = 1.2, fill = NA, col = 2) +
  scale_fill_gradient(low = "black", high = "white",
                      breaks = c(0, 10, 20, 30),
                      labels = c(0, 10, 20, ">30")) +
  labs(fill = "AWP\n(Mg/ha/yr)", x = "", y = "") +
...
ggpubr::ggarrange(gagb, gawp, labels = "auto", nrow = 2)
```

# Supplementary material

```{r map-habitat, height = 5, width = 10, fig.cap= "Topologically defined habitat map of the 50-ha plot. Source: Dalling et al., 2012"}
ggplot(bci_habitat, aes(x = x, y = y, fill = habitat)) +
  geom_raster() +
  coord_equal()+
  labs(fill = "Habitat", x = "", y = "") +
  expand_limits(x = c(0, 1000), y = c(0, 500)) +
  scale_x_continuous(expand = c(0, 0)) +
  scale_y_continuous(expand = c(0, 0)) +
  theme_bw()
...
dfhab <- df_crown[, .(agb = paste(round(mean(agb)), "+/-",
                                  round(sd(agb)/sqrt(length(agb)))), 
                      awp = paste(round(mean(awp), 2), "+/-",
                                  round(sd(awp)/sqrt(length(awp)), 2))), .(habitat)]

knitr::kable(dfhab, col.names = c("Habitat", "AGB (Mh/ha)", "AWP (Mg/ha/yr)"), caption = "Mean (+/- standard deviation) of aboveground biomass and aboveground woody productivity by topographically defined habitat (as defined in Dalling et al., 2012). The AGB and AWP of individual trees were distributed among 5x5m pixels based on the area of intersection between the pixel and their crown (using crown allometry of Martinez Cano et al., 2019).")
```

```{r fig-habitat, fig.height=4, fig.width=7, fig.cap="Aboveground biomass and aboveground woody productivity by topographically defined habitat (as defined in Dalling et al., 2012). Dots represnet the mean, and error bars the standard errors across 20x20m quadrats. The AGB and AWP of individual trees were distributed among 5x5m pixels based on the area of intersection between the pixel and their crown (using crown allometry of Martinez Cano et al., 2019). Results are consistent with a similar analysis by Chave and colleagues (2003)."}
...
ggplot(dfhab2, aes(x=habitat, y = mean, ymin = mean-se, ymax = mean+se, col = habitat)) +
  geom_pointrange() +
  labs(x = "", y = "") +
  facet_wrap(~variable, scales = "free", strip.position = "left") +
  theme_classic() +
