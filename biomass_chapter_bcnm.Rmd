...
> something about plot layout, what trees are measured (eg dbh threshold?)
...
Biomass stocks and fluxes exhibit high small-scale spatial variability in the 50 ha BCI plot [@Rejou-Mechain2014], primarily due to high turnover rates, creating small-scale canopy openings in the plot [ref]. This high spatial noise in the data, in addition to the relative homogeneity of the geologic substrate in the 50 ha plot [ref], makes it difficult to identify clear patterns in the spatial distribution of biomass (Figure \@ref(fig:fig4-spatial)). Topography explains little variation in aboveground biomass stocks across the plot [@Ledo2016], but some differences across geological substrates have been detected [@DeWalt2004]. Liana abundance has been shown to be negatively correlated with aboveground biomass [@Ledo2016; ref chapters on liana removal experiments, on ecology of lianas]. Finally, a small portion of the 50-ha plot is composed of secondary forest (outline in future Figure 4?) and has thus lower aboveground biomass. 

*	Spatial variation in BCNM AGB stocks and fluxes from multiple plots, and experimental results 

The 50 ha plot is not representative of the island of Barro Colorado, let alone the entire Barro Colorado National Monument: it is located in a particularly flat part of the island, with a relatively homogeneous geological substrate and fairly old forest. This contrasts with the rest of the island, which has a greater diversity of soil, topography, and forest age [@Mascaro2011]. The 50 ha plot has a larger mean biomass then the rest of the Barro Colorado National Monument (ref figure with 1-ha plots + large BCI plots). 

    + Variation among other plots in the BCNM, as estimated from 1-ha plots and the larger plots based on trees> 20 cm only 

    + Possible TABLE or figure – AGB stocks and fluxes for different plots in BCNM  besides the 50 ha plot, 

        - Large plots on BCI censused only for trees > 20 cm dbh (draw on work by Tori Meakem)
        
        - 1-ha plots, previous analyses by Camille
...
*	Experimental results from BCNM work on AGB stocks and fluxes 

    + [one sentence on each, and refs to associated chapters)
...
r <- raster::rasterFromXYZ(bci_habitat[, c("x", "y", "secondary")])
...
gagb <- ggplot(df_crown) + 
  ...
  geom_sf(data = sec_poly, size = 1.5, fill = NA, col = 2) +
