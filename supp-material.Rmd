...
biomass_allometry <- "chave14"

use_local_height_allometry <- TRUE

use_kohyama19_correction <- TRUE

lower_ddbh_threshold <- -2.5
upper_ddbh_threshold <- 7.5
hom_change_threshold <- 1.0
```
...
mean_agb <- df_small[year==1990 & dbh < 5.5, 
                     .(agb_m = mean(agb), 
                       ba_m = mean(ba)), 
                     .(dbh_r)]

# add mean AGB and BA to the data frame
df_small <- merge(df_small, mean_agb, all = TRUE, by = "dbh_r")[order(stemID, year)]

# recalculate AGB growth
df_small[, `:=`(Dagb_small = c(diff(agb_m)/diff(year), NA), 
                Dba_small = c(diff(ba_m)/diff(year), NA)), .(stemID)]

# substitute AGB growth with new values in original data frame
df_small <- df_small[year==1985 & dbh < 5.5, c("stemID", "Dagb_small", "Dba_small")]

df_stem <- merge(df_stem, df_small, all.x = TRUE)
df_stem[!is.na(Dagb_small) & year == 1985, `:=`(Dagb = Dagb_small, 
                                                Dagb_t = Dagb_small, 
                                                Dba = Dba_small, 
                                                Dba_t = Dba_small)]
df_stem[, `:=`(Dagb_small = NULL, Dba_small = NULL)]
``` 
...
df_stem[, hom_change := c(abs(diff(hom)) > hom_change_threshold , NA), .(stemID)]
df_stem[, hom_change := ifelse(is.na(hom_change) & !is.na(Ddbh),TRUE, hom_change)]
df_stem[, outlier := !is.na(Ddbh) & (hom_change | Ddbh > upper_ddbh_threshold | Ddbh < lower_ddbh_threshold)]

# relative AGB change per plot for survivors , by size class
df_stem[, `:=`(tot_rawp = sum(Dagb[!outlier],na.rm=T)/sum(agb[!outlier]),
               tot_rawp_t = sum(Dagb_t[!outlier],na.rm=T)/sum(agb_t[!outlier]),
               tot_rbap = sum(Dba[!outlier],na.rm=T)/sum(ba[!outlier]),
               tot_rbap_t = sum(Dba_t[!outlier],na.rm=T)/sum(ba_t[!outlier])),
        .(size = cut(dbh, c(0, 10, 20, 50, 500)))]  

# substitute outliers' AGB and BA change
df_stem[, Dagb_s := ifelse(outlier == TRUE, tot_rawp*agb, Dagb)]
df_stem[, Dba_s := ifelse(outlier == TRUE, tot_rbap*ba, Dba)]

# substitute outliers' AGB and BA change
df_stem[, Dagb_t_s := ifelse(outlier == TRUE, tot_rawp_t*agb_t, Dagb_t)]
df_stem[, Dba_t_s := ifelse(outlier == TRUE, tot_rbap_t*ba_t, Dba_t)]

df_stem <- df_stem[, -grep("tot_r", colnames(df_stem)), with = FALSE]
```
...
We excluded AWP from trees recruited with DBH in cm greater than 10 + 7.5 * census interval in years, on the assumption that these are not truly new recruits.
...
large_recruits <- recr[dbh > 10 + upper_ddbh_threshold*prevqdt & StemTag==1 & Family != "Arecaceae", 
...
recr[, `:=`(qyear = prevqyear, 
            ...
            Dagb_s = agb/prevqdt,
            Dagb_t_s = agb_t/prevqdt,
            Dba = ba/prevqdt,
            Dba_t = ba_t/prevqdt,
            Dba_s = ba/prevqdt,
            Dba_t_s = ba_t/prevqdt
)]
...
df_stem[, `:=`(
  ...
  DbaM = ba * (!is.na(census_mort) & census_year == census_mort) / qdt,
  DbaM_t = ba_t * (!is.na(census_mort) & census_year == census_mort) / qdt
)]
...
df_stem_melt <-
  data.table::melt(
    ...
    measure.vars = grep("chave|Dagb|agb_|ba|Dba", colnames(df_stem)),
    variable.name = "method",
  )

df_stem_melt <- subset(df_stem_melt, !is.na(value) & quadrat != "")

# create variables: agb, awp, awm
df_stem_melt[, variable := c("stock", "gain", "loss")[1 + 
                                                        grepl("Dagb|Dba", method) + 
                                                        grepl("agbM|baM", method)]]

# rename methods
df_stem_melt[, method := gsub("_s", "+subs", method)]
df_stem_melt[, method := gsub("_t", "+taper", method)]  
df_stem_melt[, method := gsub("DagbM|Dagb|agb", "chave14_h", method)]
df_stem_melt[, method := gsub("DbaM|Dba", "ba", method)]
```


```{r repeat-some-values}
## for outlier substitution: add chave14_h for agb and awm
df_stem_melt_subs <- df_stem_melt[variable %in% c("stock", "loss") & method == "chave14_h"]
df_stem_melt_subs[, method := "chave14_h+subs"] 
df_stem_melt <- rbind(df_stem_melt, df_stem_melt_subs)

# repeat for taper + outlier substitution
df_stem_melt_subs <- df_stem_melt[variable %in% c("stock", "loss") & method == "chave14_h+taper"]
df_stem_melt_subs[, method := "chave14_h+taper+subs"] 
df_stem_melt <- rbind(df_stem_melt, df_stem_melt_subs)

## for outlier substitution: add ba and bam
df_stem_melt_subs <- df_stem_melt[variable %in% c("stock", "loss") & method == "ba"]
df_stem_melt_subs[, method := "ba+subs"] 
df_stem_melt <- rbind(df_stem_melt, df_stem_melt_subs)

# repeat for taper + outlier substitution
df_stem_melt_subs <- df_stem_melt[variable %in% c("stock", "loss")  & method == "ba+taper"]
df_stem_melt_subs[, method := "ba+taper+subs"] 
df_stem_melt <- rbind(df_stem_melt, df_stem_melt_subs)
```

```{r add-number-trees}
## all
df_stem_melt_n <- df_stem_melt[method=="chave14_h" & DFstatus=="alive"]
df_stem_melt_n[, `:=`(variable = "stock", method = "n", value = 1, stemID = 0)] 
df_stem_melt_n <- unique(df_stem_melt_n)
df_stem_melt <- rbind(df_stem_melt, df_stem_melt_n)

## above 10 cm dbh
df_stem_melt_n10 <- df_stem_melt[method=="chave14_h" & DFstatus=="alive"& size != "[1,10]"]
df_stem_melt_n10[, `:=`(variable = "stock", method = "n10", value = 1, stemID = 0)] 
df_stem_melt_n10 <- unique(df_stem_melt_n10)
df_stem_melt <- rbind(df_stem_melt, df_stem_melt_n10)
```
...
aggregate_agb_dyn <- function(data = df_stem_melt, 
  ...
  if (nboot > 0) {
    ...
    dfdt <- df[order(year), .(year = unique(year), dT = c(diff(unique(year)), NA)), .(quadrat)]
    df <- merge(df, dfdt, by = c("quadrat", "year"))
    
    if (delta) # get change in agb
      df[order(year), delta := c(diff(stock)/diff(year), NA), .(group, quadrat)]
    ...
    if (delta) # get change in agb
      df[order(year), delta := c(diff(stock)/diff(year), NA), .(group)]
  }
  
  if (kohyama) {
    # avoid NAs and change them into zeros (no observation = null flux)
    df[is.na(gain) & year != max(year), awp := 0]
    df[is.na(loss) & year != max(year), awm := 0]
    # apply Kohyama correction 
    df[, awp := kohyama_correction(stock, gain, loss, dT, output = "prod")]
    df[, awm := kohyama_correction(stock, gain, loss, dT, output = "mort")]
  }
  
  # to long format
  df <- data.table::melt(df, 
                         measure.vars = grep("gain|loss|stock|delta", colnames(df), value = TRUE), 
...
df_plot_kohyama <- subset(aggregate_agb_dyn(
  ...
  nboot = 1000, 
  delta = TRUE
), variable %in% c("gain", "loss"))
df_plot_kohyama[, method := paste0(method, "+kohyama")]

## same methods as sullivan et al 2020
df_plot_sullivan <- aggregate_agb_dyn(
  data = df_stem_melt[size != "[1,10]" & method == "chave14_h"],
  group = "method",
  area = 50,
  kohyama = TRUE, 
  nboot = 1000, 
...
df_plot_sullivan[, method := "chave14_h_dbh10+kohyama"]

df_plot <- rbind(df_plot, df_plot_kohyama, df_plot_sullivan)
```
...
df_species_agb <-
  aggregate_agb_dyn(
    data = df_stem_melt[method == "chave14_h+subs"],
    "sp",
    area = 50,
    nboot = 1000,
    kohyama = use_kohyama19_correction
  )
df_species_agb$method <- "chave14_h+subs"

df_species_ba <-
  aggregate_agb_dyn(
    data = df_stem_melt[method == "ba+subs"],
    "sp",
    area = 50,
    nboot = 1000,
    kohyama = use_kohyama19_correction
  )
df_species_ba$method <- "ba+subs"

df_species_n <-
  aggregate_agb_dyn(
    data = df_stem_melt[method == "n"],
    "sp",
    area = 50,
    nboot = 1000,
    kohyama = FALSE
  )
df_species_n$method <- "n"

df_species_n10 <-
  aggregate_agb_dyn(
    data = df_stem_melt[method == "n10"],
    "sp",
    area = 50,
    nboot = 1000,
    kohyama = FALSE
  )
df_species_n10$method <- "n10"

df_species <- rbind(df_species_agb, df_species_ba, df_species_n, df_species_n10)
``` 
...
```{r aggr-by-pft}
df_ruger <- merge(df_stem_melt[method == "chave14_h+subs"], 
                  ruger_data[, c("sp", "PFT")], by = "sp", all.x = TRUE)

df_pft <- aggregate_agb_dyn(data = df_ruger, "PFT", area = 50, nboot = 1000, kohyama = use_kohyama19_correction)
``` 

#### Test other definition of plant functional types: Rubio et al 2022
...
```{r aggr-by-fg-rubio}
df_rubio <- merge(df_stem_melt[method == "chave14_h+subs"], 
                  ruger_data[, c("sp", "PFT")], by = "sp", all.x = TRUE)

df_pft2 <- aggregate_agb_dyn(data = df_rubio, "PFT", area = 50, nboot = 1000)
``` 


### By habitat 

We first merged the habitat data with the census data based on quadrat information, then aggregated 

```{r get-bci-habitats}
load("bci_habitat.rda")

# change quadrat name in census data to match habitat data
df_stem_melt[, q20 := as.numeric(quadrat)]

## Merge with census data
df_stem_habitat <- merge(df_stem_melt[method == "chave14_h+subs"],
                         bci_habitat[, c("q20", "habitat")], by = "q20", all.x = TRUE)

# aggregate, unit area = area of a quadrat (in ha)
df_hab <-
  aggregate_agb_dyn(
    data = df_stem_habitat,
    "habitat",
    area = 0.04,
    nboot = 10,
    kohyama = use_kohyama19_correction
  )
df_hab <- subset(df_hab, !is.na(habitat))

# divide by number of quadrats in each habitat
df_hab[, `:=`(tot = tot/sum(bci_habitat$habitat==habitat), 
              lwr = lwr/sum(bci_habitat$habitat==habitat), 
              upr = upr/sum(bci_habitat$habitat==habitat)), .(habitat)]
```
...
<!-- ### For visualization: pixels of 2x2m; only 2010 -->

<!-- ```{r crown-distribution-visualisation} -->
...
<!-- df_crown_vis <- dist_crowns(wpix = 2, -->
<!--                             gx = dftemp$gx, -->
<!--                             gy = dftemp$gy, -->
...
### For analysis: per quadrat, then aggregate values per habitat

We first distribute AGB, AWP and AWM values of each tree over its estimated crown area, and intersect it with 20x20m quadrats. We apply this to each census year and census interval. 

```{r crown-distribution-analysis}
# crown allometry from Martinez Cano et al., 2019
df_stem[, rcrown := sqrt((0.57 * dbh_t ^ 1.34)/pi) ]

df_crown_list <- lapply(seq(1985, 2015, 5), function(cns) {
  df <- df_stem[census_year==cns & !is.na(gx)]
  dist_crowns(wpix = 20, gx = df$gx, gy = df$gy,
              rcrown = df$rcrown,
              values = list(df$agb, df$Dagb_s, df$DagbM))
})

names(df_crown_list) <- seq(1985, 2015, 5)
df_crown_hab <- data.table::rbindlist(df_crown_list, idcol = "year")
df_crown_hab[, year := as.numeric(year)]

colnames(df_crown_hab)[colnames(df_crown_hab) == paste0("value", 1:3)] <- c("agb", "awp", "awm")
```

We then merge these crown-distributed values with the habitat information. 

```{r merge-crown-dist-with-habitat}
load("bci_habitat.rda")
...
## Merge with census data
df_crown_hab <- merge(df_crown_hab, bci_habitat, by = c("x", "y"), all = TRUE)

df_crown_hab[order(year), dagb := c(diff(agb)/diff(year), NA), .(q20)]

if (use_kohyama19_correction) {
  # avoid NAs and change them into zeros (no observation = null flux)
  df_crown_hab[is.na(awp) & year != max(year), awp := 0]
  df_crown_hab[is.na(awm) & year != max(year), awm := 0]
  # apply Kohyama correction 
  df_crown_hab[order(year), dT := c(diff(year), NA), .(q20)]
  df_crown_hab[, awp := kohyama_correction(agb, awp, awm, dT, output = "prod")]
  df_crown_hab[, awm := kohyama_correction(agb, awp, awm, dT, output = "mort")]
}

# to long format
df_crown_hab <- data.table::melt(df_crown_hab, 
                                 measure.vars = grep("agb|aw", colnames(df_crown_hab), value  = TRUE), 
                                 id.vars = c("habitat", "q20", "year"))
df_crown_hab <- subset(df_crown_hab, !is.na(value))

# bootstrap 
n = 1000
if (n > 0) {
  df_crown_hab <- df_crown_hab[, .(tot = sum(value),
                                   lwr = bootstrap_q(value, n, pb = 0.025), 
                                   upr = bootstrap_q(value, n, pb = 0.975)), 
                               .(habitat, year, variable)]
}

# divide by number of quadrats in each habitat
df_crown_hab[, `:=`(tot = tot/sum(bci_habitat$habitat==habitat), 
                    lwr = lwr/sum(bci_habitat$habitat==habitat), 
                    upr = upr/sum(bci_habitat$habitat==habitat)), .(habitat)]
```
...
save(df_plot, df_size, df_pft, df_pft2, df_hab, df_species, df_crown_hab, file = "cache.rda")
```
