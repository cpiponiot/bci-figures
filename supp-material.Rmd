...
df_stem[, `:=`(
  DagbM = agb * (!is.na(year_mort) & census_year == year_mort) / dTq,
...
```{r plot-level-stocks-fig, fig.height = 3, fig.width=9}
library(ggplot2)
load("cache.rda")
...
levels(dfig$variable) <- c("AGB (Mg/ha)", "AWP (Mg/ha/yr)", "AWM (Mg/ha/yr)")
...
```{r plot-level-stocks-tab, fig.height = 3, fig.width=9}
# prepare data for the figure
...
```{r fig3-groups, fig.height = 10, fig.width = 10}
