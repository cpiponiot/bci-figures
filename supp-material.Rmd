...
We define a function that distributes the aboveground biomass and woody productivity of all trees across their crown. It takes the radius of crown and the height of pixels (both in m) as inputs. xxx




```{r area-inter-circle-square}
...
# height h) with a circle of center cx, cy and radius r

area_circle_square <- function(x0, y0, h, cx, cy, r)
{
...
some trees' crowns will fall outside the plot, etc

```{r}
# get crown distributed aboveground biomass density
# create pixel grid
dist_crowns <- function(hpix, # height of pixels in m
  ...
  xmin <- floor(min(gx - rcrown) / hpix) * hpix
  xmax <- ceiling(max(gx + rcrown) / hpix) * hpix
  ymin <- floor(min(gy - rcrown) / hpix) * hpix
  ymax <- ceiling(max(gy + rcrown) / hpix) * hpix
  ...
  # distance between the center of the pixel and its corners = sqrt(2)/2*hpix)
  
  dfpix <-
    grid[, .(stems = which(sqrt((x - gx) ^ 2 + (y - gy) ^ 2) < rcrown + sqrt(2) / 2 * hpix)), .(pixID, x, y)]
  ...
  dfpix[, parea := area_circle_rect(x - hpix / 2, x + hpix / 2, y - hpix / 2, y +
                                      hpix / 2, gx, gy, rcrown)/(rcrown^2*pi), .(stems, pixID)]
  ...
  for (i in length(values)) {
    ...
    dfdisti <- 
      dfpix[, .(value = sum(parea * value, na.rm = TRUE) / (hpix / 100) ^ 2), 
          .(x, y)]
...
The radius of the crown is calculated based on the multi-species
